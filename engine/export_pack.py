...
def write_h5_pack(dest: Path, title: str, graph: dict, drive, static_dir: Path | None = None) -> Path:
    folder = write_pack(dest, title, graph, drive, None, None)
    static_dir = Path(static_dir) if static_dir else None
    if static_dir and static_dir.exists():
        for name in ("play.html", "player.js", "host.js", "styles.css"):
            src = static_dir / name
            if src.exists():
...
<head>
  ...
  <title>{title}</title>
  <style>
    html,body {{ margin:0;height:100%;background:#0b0d12;color:#ece7d8;font-family:sans-serif; }}
...
<body>
  <div class="boot">
    <div>
      <h1 style="color:#e7c07a">{title}</h1>
      <p>H5 pack. Serve this folder then open play.html or index.</p>
      <p><a href="play.html">Open player</a> · <a href="game.json">game.json</a></p>
    </div>
  </div>
  <script>
    // file:// fallback: play.html still talks to /api/pack when hosted by CineMaker.
    // For static hosting, drop a tiny rewrite or open via Play-H5.bat.
  </script>
</body>
</html>
"""
    (folder / "index.html").write_text(index, encoding="utf-8")
    (folder / "Play-H5.bat").write_text(
